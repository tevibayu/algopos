...
class EloquentUserRepository implements UserContract {
	 ...
	 * @return mixed
	 */
	public function getUsersPaginated($per_page, $status = 1, $is_search = false, $search = '', $order_by = 'id', $sort = 'asc') {
	    if ($is_search) {
                $users = User::where('status', $status)
                                ->search(
                                    $search,
                                    array('name', 'email'),
                                    array('confirmed' => array(1 => 'yes', 0 => 'no'))
                                )->orderBy($order_by, $sort)
                                ->paginate($per_page);
            } else {
                $users = User::where('status', $status)->orderBy($order_by, $sort)->paginate($per_page);
            }
            return $users;
	}
	 ...
	 */
	public function getDeletedUsersPaginated($per_page, $is_search = false, $search = '') {
            if ($is_search) {
	 ...
	 */
	public function create($input, $roles, $permissions,$request) {
		$user = $this->createUserStub($input);

		if ($user->save()) {
			$activities = new Activities;
            $activities->log_activity($request,'Create User id '.$user->id,'User Management');
