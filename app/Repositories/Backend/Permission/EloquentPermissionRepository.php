...
class EloquentPermissionRepository implements PermissionRepositoryContract {
	 ...
	 * @return mixed
	 */
	public function getPermissionsPaginated($per_page, $is_search = false, $search = '', $order_by = 'display_name', $sort = 'asc') {
            if ($is_search) {
                $permissions = Permission::with('roles')
                                        ...
                                        ->search(
                                            ...
                                            array('system' => array(1 => 'yes', 0 => 'no'))
                                        )->paginate($per_page);
            } else {
                $permissions = Permission::with('roles')->orderBy($order_by, $sort)->paginate($per_page);
            }
            return $permissions;
	}
	 ...
	 * @throws GeneralException
	 */
	public function create($input, $roles,$request) {
		...
		if ($permission->save()) {
	        ...
	        $activities->log_activity($request,'Create Permission id '.$permission->id,'User Management');
	...
	public function update($id, $input, $roles) {
        ...
        if ($permission->name != $input['name']) {
			if (Permission::where('name', '=', $input['name'])->first()) {
				throw new GeneralException('That name belongs to a different permission name.');
            }
		}
