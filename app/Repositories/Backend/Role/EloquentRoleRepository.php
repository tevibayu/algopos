...
class EloquentRoleRepository implements RoleRepositoryContract {
	 ...
	 * @return mixed
	 */
	public function getRolesPaginated($per_page, $is_search = false, $search = '', $order_by = 'sort', $sort = 'asc') {
            if ($is_search) {
                $roles = Role::with('permissions')
                            ->orderBy($order_by, $sort)
                            ->search(
                                $search,
                                array('name', 'login_destination')
                            )->paginate($per_page);
            } else {
                $roles = Role::with('permissions')->orderBy($order_by, $sort)->paginate($per_page);
            }
            return $roles;
	}
	...
	public function create($input,$request) {
		...
		$role->sort = isset($input['sort']) && strlen($input['sort']) > 0 && is_numeric($input['sort']) ? (int)$input['sort'] : 0;
                $role->login_destination = $input['login_destination'];
		...
		if ($role->save()) {
            ...
            $activities->log_activity($request,'Create Role with id '.$role->id,'User Management');
			if (! $all) {
	...
	public function update($id, $input) {
        ...
        if ($role->name != $input['name']) {
			if (Role::where('name', '=', $input['name'])->first()) {
				throw new GeneralException('That name belongs to a different role name.');
            }
		}
