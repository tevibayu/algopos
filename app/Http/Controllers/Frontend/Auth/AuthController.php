...
class AuthController extends Controller
{
    ...
    public function postLogin(LoginRequest $request)
    {
        ...
        try {
            ...
            $is_log = config('log.login_logout');
            if ($is_log) {
                $activities = new Activities;
                $activities->log_activity($request,'Login','Login');
            }
            
            if (count(access()->user()->roles)) {
                return redirect()->intended(access()->user()->roles[0]['login_destination'])
    ...
    public function getLogout(Request $request)
    {
        ...
        if ($is_log) {
            $activities = new Activities;
            $activities->log_activity($request,"Logout","Logout");
        }
         
        session()->forget('id_location');
        session()->forget('id_location_override');
        $this->auth->logout();
